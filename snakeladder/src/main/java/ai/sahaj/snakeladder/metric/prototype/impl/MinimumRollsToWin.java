...
@Component("minRollsToWin")
public class MinimumRollsToWin implements MetricGenerator<StatsData<Integer>> {
	...
	public MetricResponse<StatsData<Integer>> generateMetric(String simulationId, MetricRequest statsReq) {
		...
		Optional<RollStats> optRollStats = rollStats.stream().min(Comparator.comparingInt(RollStats::getRollCount));
		...
		metricResponse.setMetricName(Stats.RTWMIN);
		metricResponse.setMetricDescription("Minimum number of rolls needed to win");
